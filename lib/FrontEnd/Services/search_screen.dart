...
class _SearchScreenState extends State<SearchScreen> {
  ...
  @override
  void initState() {
    this._initialDataFetchAndCheckUp();
    super.initState();
  }
      ...
      child: Row(
        ...
        children: [
          Column(
            ...
            children: [
              Text(this._sortedAvailableUsers[index].values.first.toString().split('[user-name-about-divider]')[0], style: TextStyle(color: Colors.orange, fontSize: 20.0),),
              Text(this._sortedAvailableUsers[index].values.first.toString().split('[user-name-about-divider]')[1], style: TextStyle(color: Colors.lightBlue, fontSize: 15.0),),
            ],
          ),
          TextButton(
            style: TextButton.styleFrom(
                shape: RoundedRectangleBorder(
                  ...
                  side: BorderSide(color: _getRelevantButtonConfig(connectionStateType: ConnectionStateType.ButtonBorderColor, index: index),),
                )
            ),
            child: _getRelevantButtonConfig(connectionStateType: ConnectionStateType.ButtonNameWidget, index: index),
                ...
                if(mounted){
                  setState((){
                    this._myConnectionRequestCollection.add({
                      this._sortedAvailableUsers[index].keys.first.toString(): OtherConnectionStatus.Request_Pending.toString(),
                    });
                  });
                }
  ...
  dynamic _getRelevantButtonConfig({required ConnectionStateType connectionStateType, required int index}){
    ...
    this._myConnectionRequestCollection.forEach((element) {
      if(element.keys.first.toString() == this._sortedAvailableUsers[index].keys.first.toString()){
        _isUserPresent = true;
    ...
    if(_isUserPresent){
      ...
      if(_storeStatus == OtherConnectionStatus.Request_Pending.toString() ||
        ...
        if(connectionStateType == ConnectionStateType.ButtonNameWidget) {
          ...
          return _storeStatus == OtherConnectionStatus.Request_Pending.toString()
              ? ConnectionStateName.Pending.toString()
              : ConnectionStateName.Accept.toString();
        }
