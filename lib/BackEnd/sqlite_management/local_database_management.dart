...
class LocalDatabase {
  ...
  Future<Database> initializeDatabase() async {
    ...
    final String path = newDirectory.path + '/saci_local_storage.db';
  ...
  Future<void> createTableToStoreImportantData() async {
    try {
      ...
      await db.execute(
          "CREATE TABLE ${this._importantTableData}($_colUserName TEXT PRIMARY KEY, "
              ...
              "$_colAccCreationDate TEXT, $_colAccCreationTime TEXT)");
    ...
    try {
      ...
      if (purpose != 'insert') {
        final int updateResult = await db.rawUpdate(
            "UPDATE $_importantTableData SET $_colToken = '$userToken', "
                "$_colAbout = '$userAbout', $_colUserMail = '$userMail', "
                "$_colAccCreationDate = '$userAccCreationDate',"
                " $_colAccCreationTime = '$userAccCreationTime' WHERE "
